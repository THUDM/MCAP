...
def test_baseline(model, user_dict, n_params, deg, mode='test'):
    ...
    for u_batch_id in range(n_user_batchs):
        ...
        user_batch = torch.LongTensor(test_users[start: end]).to(device)
        ...
        for i_batch_id in range(n_item_batchs):
            ...
            for j in range(user_batch.size(0)):
                item_batch.append([i for i in range(i_start, i_end)])
            item_batch = torch.LongTensor(item_batch).to(device)
            i_rate_batch = model.forward_multi_items(user_batch, item_batch)
            
            rate_batch[:, i_start:i_end] = i_rate_batch.detach().cpu()
        ...
        for re in batch_result:
            user_recall_result[user_batch[i]] = re['recall'].tolist()[0]
